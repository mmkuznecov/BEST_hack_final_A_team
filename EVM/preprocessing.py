...
def read_video(path):
    cap = cv2.VideoCapture('outpy_Trim.mp4')
    ...
    while cap.isOpened():
        ...
        if len(face_rects) > 0:
            ...
            if roi_frame.size != img.size:
                ...
                img_copy = cv2.copyMakeBorder(img_copy,top=420,bottom=420,left=0,right=0,borderType=cv2.BORDER_CONSTANT)
                print(type(img_copy))
                print(img_copy[0][0])
                print(frame.shape)
                print(img_copy.shape)
                video_frames.append(img_copy)
